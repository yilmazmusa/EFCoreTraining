...
//Ben deniz(Musa) genellikle bu özelliği yeni çıkan EF Core özelliklerini test edebilmek için kullanıyorum. 
...
//Test ve pre-prod uygulamalarda gerçek/fiziksel veritabanları oluşturmak ve yapılandırmak yerine tüm veritanını bellekte modelleyebilir ve gerekli işlemleri sanki gerçek bir veritabanında çalışıyor gibi orada gerçekleştirebiliriz.
...
await context.Persons.AddAsync(new() { Name = "Musa", Surname = "Yılmaz" });
await context.SaveChangesAsync();

var persons = await context.Persons.ToListAsync();
 var eklenen_person =  await context.Persons.Where(p => p.Id == 1).ToListAsync();
...
class ApplicationDbContext : DbContext
{
    ...
    {
        optionsBuilder.UseInMemoryDatabase("ExampleDatabase"); //In-Memory de ExampleDatabase adında bir VT oluşturuduk.In-Memory de oludğu için migration işlemleri yapmıyoruz.
    }
}